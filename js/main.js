...
// icons end
  ...
  function theFortune() {
    var fortuneNum = getRandomInt(0, 60),
        ...
        tl = new TimelineMax(),
        fortune = "<h2>" + fortunes[13] + "</h2>",
        deterFortune = $("#deterFortune").html(fortune),
        moveme = "-10%";

        // console.log(fortunes[fortuneNum]);
        console.log(fortunes[13]);
        console.log(fortunes[13].length);

        // 10, 15, 13

        if (fortunes[13].length < 40) {
          moveme = "-370%";
        } else if (fortunes[13].length > 80) {
          moveme = "-150%";
        } else {
          moveme = "-260%";
        }

    tl.fromTo(deterFortune, 0.5, {autoAlpha:0, scale: 0.2, y: moveme}, {autoAlpha: 1, scale: 1.0, y: moveme, ease: Power2.easeInOut});
